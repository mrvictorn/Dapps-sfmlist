...
#    @connect2Node ethNodeAddress
  ...
  setDeployedContract: (oContract)=>
    # minimal checks
    name = oContract?.name
    contractObj = oContract?.contract
    if name and (_.isString name) and contractObj
      @deployedContracts[name] = contractObj
      @emit "contractReady#{name}", contractObj
      return contractObj
    else
      return null
  ...
  setDeployedContracts: ()=>
    ...
    contracts2load.forEach (contractData)->
      abi = contractData.abi
      addr = contractData.address
      if abi and addr
        oContract = _loadContractByAddress abi,addr
        self.setDeployedContract
  ...
  startObservingDeployedContracts: ()=>
    ...
    if Meteor.isServer
      ...
      self.systemContracts.after.insert (userId, doc) ->
        console.log 'contract inserted:', doc
      self.systemContracts.after.update (userId, doc) ->
          console.log 'contract updated:', doc
    else
...
###
  Usage of package:
  on both client and server add
  Meteor.startup ()->
    contracts.Bootstrap(nodeAddress)

  This will create global object
###
